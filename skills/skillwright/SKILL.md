...
Guide the writing of a skill that an agent can read cheaply, retrieve from reliably, and execute without guessing.
...
The description is the only part loaded in every session, roughly 50 tokens of always-paid context, and it alone decides whether the skill fires. Include, in one or two sentences:
...
| Level | Loaded | Budget |
|-------|--------|--------|
| description | always | ~50 tokens, spend them on triggers |
...
Getting this split right saves more context than any formatting choice inside a file. If the body explains one ecosystem or scenario for more than ~30 lines, that content wants to be a reference file. Give sibling reference files an identical section skeleton so an agent that learned one can navigate the others blind.

## Step 5: Write the body

Open with the workflow in one line, then a numbered non-negotiables block: attention is strongest at the start, and later text can cite "non-negotiable 3" instead of restating it. Number sections (§2.2 style) when references cross-link them; anchors act as pointers and cut repetition.
...
| Parallel, order-independent items | bullets | The only case bullets win |
| Causal or conditional logic | one tight sentence | "A → B because C unless D" dies when fragmented |
| Expected output | literal template, exact strings to say | Showing beats describing; collapses variance |
...
- [ ] Body opens with the workflow and a non-negotiables block
