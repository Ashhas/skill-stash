...
- **1c. Scope buckets:** how this ecosystem categorises a dependency; this becomes the report's Scope column. Examples: Gradle `implementation`/`api`/plugin/version-catalog entry; Python runtime vs dev/extras group; Cargo `dependencies`/`dev-dependencies`/workspace-inherited; pub `dependencies`/`dev_dependencies`. Every candidate gets exactly one bucket.
...
- **2a. Runtime compatibility:** where the module declares its runtime version (Java toolchain, `requires-python`, `rust-version`, the `go` directive, the Dart SDK constraint) and the check that a new dependency version still supports it. Requires a newer runtime than the module uses → BLOCKED.
- **2b. Coupling matrix:** the ecosystem's framework couplings, meaning the groups that must move together. Derive from the project's actual stack (its BOM/platform equivalents, plugin ↔ runtime pairs, generated-code tools). If unsure about a pairing, default to BLOCKED rather than RISKY.
...
- **3c. Hardening rules:** if the project restricts installs (no lifecycle scripts, frozen or immutable lockfile, vendored deps, offline mirrors), respect that absolutely. A bump that needs an exception is BLOCKED pending the project's own review procedure.
...
- One commit per module; stage the manifest, lockfile, and any audited source fixes together.
- The shared rules in [SKILL.md](../SKILL.md) apply unchanged to every derived lane: classification buckets (§2.1), the release-notes-and-migration-guide discipline (§2.2), audit items (§4.3), the report format (§2.4), and the PR confirmation gate (STEP 5).
