...
- **Wanted**: the highest version satisfying the `package.json` range. `Current → Wanted` is an **in-range** bump (a `yarn up` / `npm update` picks it up without editing `package.json`). These are the SAFE candidates.
- **Latest**: the newest published version. When `Latest > Wanted`, the semver range must be widened in `package.json`, usually a **major** bump. These are the RISKY/BLOCKED candidates.
...
Cover dependencies plus devDependencies and walk transitives. Each advisory has a severity, a vulnerable range, and dependents. A vulnerable dependency is a candidate **regardless of bump type**. Flag it in classification with its severity even when it's only reachable transitively; the fix may be a `resolutions`/`overrides` entry rather than a direct bump.
...
The JS equivalent of the Maven ecosystem matrix. Bumping one side of a coupled pair without the other breaks the build or the types. Examples; verify against the project's actual stack and extend as needed:
...
| `@types/react` ↔ `react`, `@types/node` ↔ Node | The types major must match the runtime major. |
...
**Resolution drift** is the Node analog of Maven lockstep drift, and it joins the batch automatically like any Maven drift. Compare each `resolutions`/`overrides` entry against the same package's `dependencies`/`devDependencies` range:
...
- `Latest > Wanted` (the range must widen), i.e. a **major** → **RISKY or BLOCKED**; continue below. Widening a `^` range across a major is a deliberate change, never routine.
...
- Transitive-only with no direct-dep fix → propose a `resolutions`/`overrides` entry; classify RISKY (a forced transitive version can break other consumers) and note it needs a clean install plus a test pass to confirm.

### 2.d: Node audit-item sources (for SKILL.md §2.2)

The shared release-notes discipline applies unchanged: direct usage plus no migration guide → BLOCKED; transitive-only → RISKY with a flag. Node-specific grep targets for the audit items: `import` sites in `src/`, `next.config.*`, `eslint.config.*`/`.eslintrc*`, `tsconfig*.json`, `vitest.config.*`, and `playwright.config.*`.
...
| `@playwright/test` / `playwright` | The integration/e2e suite (needs the app and browsers) |
...
| Typecheck breaks after a `@types/*` or `typescript` bump | Confirm the types major matches the runtime lib major (§1.4). If it's a genuine new strictness check, treat it as RISKY audit work, not a routine bump. |
| Regeneration produces an unexplained diff | Revert the codegen-related bump and mark BLOCKED (see guardrail). |
| Formatting check fails after a prettier/eslint bump | Run the project's format-fix or lint-fix script, commit the formatting diff (analogous to `spotless:apply`). |
...
- [ ] Framework couplings checked (§1.4 / §2.b): Next ↔ React, `@types` ↔ runtime, the `vitest` set, `resolutions` pins on coupled packages
